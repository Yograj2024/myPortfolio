...
import { workExperienceData } from '../../utils/languageConstent';

const WorkExperience = ({theme}) => {

    const lanCode = getLanConfig("lanCode");
    const {workExperience, position, company, duration, description} = workExperienceData;
    ...
    return <div className ={` w-full`}>
        ...
        <div className ={`relative md:w-[95%] ${theme == "dark" ? "text-white bg-[var(--secondry-bg)]" : "bg-[#ffffff]"} rounded-[1rem] py-[1rem] px-[1rem] md:px-[1.5rem]`}>
            ...
            <h2 className ={`text-[1.5rem] ${theme !== "dark" && "text-[#8754cf]"}  capitalize font-semibold mb-[1rem]`}>{position[lanCode]}</h2>
            <strong className ={`w-full flex justify-between items-center text-[var(--main-text)]`}> 
                ...
                <span className ={`font-semibold inline-block text-[0.8rem] ms:text-[0.9rem] bg-[#9A5AF2] text-white rounded-[0.5rem] px-[1rem] py-[4px]`}>{duration[lanCode]}</span> 
            </strong>

            <ul className ={`mt-[1rem] ms:text-[1rem] text-[0.85rem] ${theme == "dark" ? "text-[var(--main-text)]" : ""}`} > 
                {
                    description[lanCode].map( (points, index) =>  <li key={index} className ={`py-[0.8rem]`}>{points}</li> )
                }
            </ul>
        </div>
